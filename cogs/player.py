...
async def generate_leaderboard_embed(db: MongoDatabaseManager, guild_id: int, user_id: Optional[int] = None, offset: int = 0, limit: int = 10) -> discord.Embed:
    ...
    try:
        ...
        offset = max(0, offset)
        limit = max(1, min(limit, 50))  # Limit maximum entries per page
        ...
        # Normalize names and prepare data with better error handling
        levels = []
        names = []
        ranks = []

        for i, member in enumerate(top):
            levels.append(member.get("habit_count", 0))
            # Better name handling with fallback
            display_name = member.get("display_name") or member.get("username") or "Unknown"
            names.append(unidecode(str(display_name))[:20])  # Ensure string and truncate
            ranks.append(offset + i + 1)
